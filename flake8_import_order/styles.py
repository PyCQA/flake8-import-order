...
RELATIVE_SET = set((IMPORT_APP, IMPORT_APP_RELATIVE))
...
class Style(object):
    ...
    def same_section(previous, current):
        ...
        both_relative = set((previous.type, current.type)) <= RELATIVE_SET
...
class Cryptography(Style):
    ...
    @staticmethod
    def key(import_):
        if import_.type in set((IMPORT_3RD_PARTY, IMPORT_APP)):
            return (
    ...
    @staticmethod
    def same_section(previous, current):
        app_or_third = current.type in set((IMPORT_3RD_PARTY, IMPORT_APP))
        same_type = current.type == previous.type
        both_relative = set((previous.type, current.type)) <= RELATIVE_SET
